...
test("buildPreviewFileName creates stable split output names", () => {
  const [segment] = parsePageRangeInput("5-8", 12);
...
test("toSplitRequestPayload maps parsed segments into the split backend shape", () => {
  const segments = parsePageRangeInput("1-3, 5", 12);
